...
DECLARE @InvoiceLineItemKey BIGINT = 0
DECLARE @TotalInvoiceLineItemRows INT =
        (
            SELECT COUNT(*)FROM @LassSalesTaxBatches
        )
DECLARE @CurrentInvoiceLineItemRow INT = 0
...
BEGIN
    DECLARE @LineItemCalculatorModule NVARCHAR(128) =
            (
                ...
                WHERE lili.InvoiceLineItemKey = @InvoiceLineItemKey --li.LineItemKey = @InvoiceLineItemKey)
            )
